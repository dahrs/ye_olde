...
- **Physical partitioning: by language, then by volume — never a fixed calendar grid.**
  ...
  └── …
  ```
- **Compute — Hugging Face Spaces, Docker SDK, free CPU tier.** A small FastAPI app (not Gradio — this is a pure JSON API, no UI needed) that on startup downloads the current Parquet + FAISS files from the Dataset repo (`hf_hub_download`) and loads them into memory/`mmap`. Free-tier Spaces sleep after inactivity and cold-start on the next request — acceptable for v1, revisit only if latency becomes a real problem.
- **Endpoints (v1, minimal):**
  ...
  - `GET /health`.
- **Code hosting — GitHub**, same repo as everything else, deployed to the Space via GitHub Actions (push-to-deploy) so the API's code lives with the rest of the project instead of only inside the Space's own git remote.
